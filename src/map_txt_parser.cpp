...
// TODO remove me windows compile fix
#ifdef WINDOWS
using uint = uint32_t;
#endif

char *find_str(uint8_t *map_txt, const char *str, int len) {
...
char *parse_objects(map_lvls *map, int level) {
  ...
  for (size_t i = 0; i < objects_size; i++) {
    ...
    if (io_strncmp(&map->objects[i], "[OBJECT BEGIN]",
                   sizeof("[OBJECT BEGIN]") - 1) == 0) {
      begin = &map->objects[i];
      i += sizeof("[OBJECT BEGIN]");
      continue;
    }
    if (io_strncmp(&map->objects[i], "obj_elev: ", sizeof("obj_elev: ") - 1) ==
        0) {
      char l = level + '0';
    ...
    if (io_strncmp(&map->objects[i], "[BEGIN INVEN ITEMS]",
                   sizeof("[BEGIN INVEN ITEMS]") - 1) == 0) {
      // Apparently a critter's inventory is stored inside their own object
      // description And those are all marked [OBJECT BEGIN], so it breaks the
      // current parsing So here we fast forward to the end of the inventory
      // list before returning to parsing
      while (i < objects_size) {
        ...
        if (io_strncmp(&map->objects[i], "[END INVEN ITEMS]",
                       sizeof("[END INVEN ITEMS]") - 1) == 0) {
          break;
        }
      }
    }
    if (io_strncmp(&map->objects[i], "[OBJECT END]",
                   sizeof("[OBJECT END]") - 1) == 0) {
      if (begin == NULL) {
        continue;
      }
...
int script_spatial(char *script_txt, int remainder, int level) {
  ...
  for (size_t i = 0; i < remainder; i++) {
    ...
    if ((spatial_tile >= 0) && (spatial_tile < 40000)) {
      spatial_level = 0;
    } else if (spatial_tile & 0x20000000) {
      spatial_level = 1;
    } else if (spatial_tile & 0x40000000) {
      spatial_level = 2;
    } else {
    ...
    while (i++ < remainder) {
      ...
      if ((io_strncmp(&script_txt[i], "scr_id: ", scr_id_len) == 0) ||
          ...
          (io_strncmp(&script_txt[i], ">>>>>>>>>>: OBJECTS", objects_len) ==
           0)) {
        return (i - 1);
      }
    }
  ...
  printf(
      "ERROR reached end of script_txt before finding SPATIAL script ending\n");
  return 0;
}
...
int script_object(char *script_txt, int remainder, int level, char *objects) {
  ...
  for (size_t i = 0; i < remainder; i++) {
    ...
    if (io_strncmp(&script_txt[i], "scr_num_local_vars: ",
                   sizeof("scr_num_local_vars: ") - 1) == 0) {
      while (script_txt[i++] != '\n') {
      }
      return i;
    }
  ...
  printf(
      "ERROR reached end of script_txt before finding OBJECT script ending\n");
  return 0;
}
...
parsed_scripts parse_scripts(map_lvls *map, char *objects, int level) {
  ...
  //     SCRIPT_TYPE_SYSTEM,  // s_system     // ==> 0 ==> seems like the most
  //     likely match SCRIPT_TYPE_SPATIAL, // s_spatial    // ==> 1
  //     SCRIPT_TYPE_TIMED,   // s_time       // ==> 2 ==> not sure how to get
  //     system or timed scripts tho SCRIPT_TYPE_ITEM,    // s_item       // ==>
  //     3     its unlikely that system/timed scripts are sorted by level tho
  //     SCRIPT_TYPE_CRITTER, // s_critter    // ==> 4     so it shouldn't be
  //     necessary to track them? I hope? SCRIPT_TYPE_COUNT, // ==> 5
  // } ScriptType;

  // [[SCRIPT]]           //  ==> immediately follows the scr_num: count for
  // each script type
  //                      //  ==> occurs every 16 script entries in each scr_num
  //                      type

  // scr_id: 50331652     // ==> matches obj_sid on the object its attached to
  //                      // ==> is not the same as an id for the script,
  //                      //     but more like an id for this particular
  //                      instance of the script
  //                      //     for that reason each object should have a
  //                      unique obj_sid matching scr_id
  //                      //     even if the same script is reused for multiple
  //                      objects
  //                      // ==> seems to be generated whenever a script is
  //                      attached
  // scr_id: 67108864     // ==> 0x04000000  ==> indicates a critter object is
  // attached? scr_id: 50331648     // ==> 0x03000000  ==> all other object
  // types? scr_id: 16777216     // ==> 0x01000000  ==> spatial script types?
  //                      // ==> 0x00 and 0x02 appear to be unused?

  /* spatial scripts have these extra fields */
  // scr_id: 16777216                     // ==> 0x01000000  ==> spatial script
  // types
  // ...
  // scr_udata.sp.built_tile: 39999       // ==> highest tile number for level
  // 0, starts at 0 scr_udata.sp.radius: 0
  /*                                         */

  // scr_udata.sp.built_tile: 536870912   // ==> 0x020000000  ==> start tile for
  // level 1 scr_udata.sp.built_tile: 1073741824  // ==> 0x040000000  ==> start
  // tile for level 2

  // [OBJECT BEGIN]
  // ...
  // obj_sid: 4294967295        //  ==> 0xffffffff  ==> no assigned script
  // ...                                            ==> otherwise matches scr_id
  // on script [OBJECT END]                                   ==> should be
  // unique per map

  // hooo boy, ok this guy is going to need a separate string for each script
  // type
  //       (just in case the scr_num entries need to be properly tracked) (they
  //       do)
  //   also spatial script type needs to be separated by which level they're
  //   located on
  //       and then re-assigned to the new level for the new map
  //   also we need to make sure any scr_id/obj_sid pair don't overlap other
  //   pairs
  ...
  char *scr_ptr[5] = {scrs.scr_num[0], scrs.scr_num[1], scrs.scr_num[2],
                      scrs.scr_num[3], scrs.scr_num[4]};
  ...
  for (size_t i = 0; i < scripts_size; i++) {
    ...
    if (io_strncmp(&map->scripts[i], "scr_id: ", sizeof("scr_id: ") - 1) == 0) {
      ...
      case (SCRIPT_SPATIAL): {
        int spatial_len =
            script_spatial(&map->scripts[i], scripts_size - i, level);
        if (spatial_len > 0) {
      ...
      case (SCRIPT_OBJECTS): {
        int object_len =
            script_object(&map->scripts[i], scripts_size - i, level, objects);
        if (object_len > 0) {
...
void check_char_scripts(parsed_scripts *scripts, char **objects,
                        script_type type) {
  ...
  for (size_t el_out = 0; el_out < 3; el_out++) {
    for (size_t i = 0; i < strlen(scripts[el_out].scr_num[type]); i++) {
      ...
      if (io_strncmp(&scripts[el_out].scr_num[type][i],
                     "scr_id: ", sizeof("scr_id: ") - 1) != 0) {
        continue;
      }
      ...
      for (size_t el_in = 0; el_in < 3; el_in++) {
        for (; i < strlen(script_ptr); i++) {
          ...
          if (io_strncmp(&scripts[el_in].scr_num[type][i],
                         "scr_id: ", sizeof("scr_id: ") - 1) != 0) {
            continue;
          }

          uint tmp =
              atoi(&scripts[el_in].scr_num[type][i + sizeof("scr_id: ") - 1]);
          if (max_id < tmp) {
            max_id = tmp;
          }
          if (scr_id == tmp) {
...
void export_map_txt(char **label_ptr_M, map_lvls *map_L, map_lvls *map_R,
                    int header, char *path) {
  if (header == -1) {
    return;
  }
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    for (size_t j = 0; j < 3; j++) {
      ...
      } else if (io_strncmp(label_ptr_M[i], map_R->label_ptr[j], NAME_LENGTH) ==
                 0) {
  ...
  for (size_t i = 0; i < 3; i++) {
    if (out_lvl[i]) {
      snprintf(out_ptr, out_lvl_size[i] + sizeof("square_elev: i\r\n\r\n"),
               "square_elev: %i\r\n\r\n%s", i, out_lvl[i]);
  ...
  for (size_t dst_lvl = 0; dst_lvl < 3; dst_lvl++) {
    ...
    for (size_t src_lvl = 0; src_lvl < 3; src_lvl++) {
      if (io_strncmp(label_ptr_M[dst_lvl], map_L->label_ptr[src_lvl],
                     NAME_LENGTH) == 0) {
        objects[dst_lvl] = parse_objects(map_L, src_lvl);
        // need to know which objects on source level when parsing object
        // scripts
        scripts[dst_lvl] = parse_scripts(map_L, objects[dst_lvl], src_lvl);
      } else if (io_strncmp(label_ptr_M[dst_lvl], map_R->label_ptr[src_lvl],
                            NAME_LENGTH) == 0) {
  ...
  spatials[0] =
      (spatial_script *)malloc(spatial_cnt_0 * sizeof(spatial_script));
  spatials[1] =
      (spatial_script *)malloc(spatial_cnt_1 * sizeof(spatial_script));
  spatials[2] =
      (spatial_script *)malloc(spatial_cnt_2 * sizeof(spatial_script));
  ...
  for (size_t elevation = 0; elevation < 3; elevation++) {
    ...
    for (size_t i = 0; i < len; i++) {
      ...
      // scr_script_idx: 221
      if (io_strncmp(&scr_ptr[i],
                     "scr_script_idx: ", sizeof("scr_script_idx:")) == 0) {
      ...
      if (io_strncmp(&scr_ptr[i], "scr_local_var_offset: ",
                     sizeof("scr_local_var_offset:")) == 0) {
      ...
      if (io_strncmp(&scr_ptr[i], "scr_num_local_vars: ",
                     sizeof("scr_num_local_vars:")) == 0) {
      ...
      if (io_strncmp(&scr_ptr[i], "scr_udata.sp.built_tile: ",
                     sizeof("scr_udata.sp.built_tile:")) == 0) {
      ...
      if (io_strncmp(&scr_ptr[i], "scr_udata.sp.radius: ",
                     sizeof("scr_udata.sp.radius:")) == 0) {
  ...
  for (size_t elevation = 0; elevation < 3; elevation++) {
    ...
    int obj_elev_len = sizeof("obj_elev: ") -
                       1; //-1 to account for the auto-appended '\0' character
      ...
      for (size_t i = 0; i < script_cnt_out; i++) {
        ...
        if (type == SCRIPT_SPATIAL) { // Spatial Scripts only
          ...
          for (size_t el_in = el_out; el_in < 3; el_in++) {
            // loop through all inside scripts on this level and actually do the
            // check
  ...
  int amt = snprintf(out_ptr, out_size - (out_ptr - (char *)out_map),
                     ">>>>>>>>>>: SCRIPTS <<<<<<<<<<\r\n\r\n\r\n"
                     "SCRS:\r\n");
  out_ptr += amt;
  ...
  for (size_t i = 0; i < 5; i++) {
    int total = scripts[0].scr_num_cnt[i] + scripts[1].scr_num_cnt[i] +
                scripts[2].scr_num_cnt[i];
    int abc = snprintf(out_ptr, out_size - (out_ptr - (char *)out_map),
                       "scr_num: %i\r\n", total);
      ...
      if (scripts[elevation].scr_num_cnt[i]) { // TODO: remove this if() check
        if (i == SCRIPT_SPATIAL) {
          for (size_t cnt = 0; cnt < scripts[elevation].scr_num_cnt[i]; cnt++) {
            // printf(
            snprintf(out_ptr, out_size - (out_ptr - (char *)out_map),
                     "\r\n"
                     "scr_id: %u\r\n"
                     "scr_next: %u\r\n"
                     "scr_flags: %u\r\n"
  ...
  int size = snprintf(out_ptr, out_size - (out_ptr - (char *)out_map),
                      ">>>>>>>>>>: OBJECTS <<<<<<<<<<\r\n\r\n"
                      "[[OBJECTS BEGIN]]\r\n");
  out_ptr += size;
